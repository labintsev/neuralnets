...
class Momentum(Optimizer):
    ...
    def step(self, w, d_w, learning_rate):
        ...
        new_velosity = self.rho * self.velocity + d_w
        w -= new_velosity * learning_rate
        self.velocity = new_velosity
...
if __name__ == '__main__':
    ...
    num_iters = 18000
    batch_size = 64
    net = NeuralNetwork([DenseLayer(n_input, n_hidden),
                                DropoutLayer(0.5),
                                BatchNormLayer(n_hidden),
                                ReLULayer(),
                                DenseLayer(n_hidden, n_output)])
    net.setup_optimizer(Momentum())
    t0 = datetime.datetime.now()
    loss_history = net.fit(x_train, y_train, num_iters=num_iters)
    ...
    Train accuracy: {net.evaluate(x_train, y_train)}   
    Test accuracy: {net.evaluate(x_test, y_test)}    
